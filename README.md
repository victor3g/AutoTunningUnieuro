...
🧬 Algoritmo Genético (GA): Evolução baseada em seleção, crossover e mutação.

🐦 Particle Swarm Optimization (PSO): Comportamento de enxame para exploração do espaço de busca.

⚡ Híbrido (GA + Pattern Search): Exploração global com GA seguida de refinamento local agressivo.
...
Entrada: Deve aceitar 10 argumentos inteiros via linha de comando.

./seu_programa.exe 10 500 30 999 50 60 70 80 90 100


Saída: Deve imprimir apenas o valor do resultado (score) na primeira linha da saída padrão (stdout).
...
Define o objetivo: max (maior valor) ou min (menor valor).

--time

3600

Tempo total de execução (em segundos). Esse tempo é dividido igualmente entre as 3 estratégias.

--exe

simulado.exe

Caminho relativo ou absoluto para o executável que será testado.

--seed

42

Semente para geração de números aleatórios (garante reprodutibilidade).
...
Para rodar o otimizador usando esse script Python como "executável":
...
Console: Logs de "NOVO MELHOR" sempre que uma solução superior é encontrada.

relatorio.md: Um arquivo contendo:
